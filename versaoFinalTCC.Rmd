...
\tab Um ponto importante sobre as infrações de velocidade é que existe uma margem de erro para medição da velocidade registrada. Para fim de registro de infração, não se utiliza a velocidade obtida pelo instrumento de medição, mas subtrai-se um valor na velocidade medida (para vias de velocidade máxima até 100km/h, o valor é 7), o que passar deste valor acima da velocidade permitida, será considerado para infração.

\tab Por exemplo, para vias de com velocidade máxima de 60 km/h, as infrações serão registradas somente quando o veículo passar pelo ponto de medição quando a velocidade calculada for de 68 km/h, registrando um excesso de 1km/h, mas para uma velocidade registrada de 67 km/h, o valor considerado é de 60km/h. O resultado dessa distinção é o uso de dois termos: Velocidade medida, que consiste na velocidade registrada e Velocidade Considerada, que é a velocidade medida menos o fator de correção. Na tabela abaixo, temos a relação entre velocidade medida e velocidade considerada. A infração é feita quando a considerada é maior que permitida na via. 
...
\tab É importante destacar que não iremos realizar uma distinção do sentido que o veículo está indo, para análises futuras, pode-se aprofundar nesta análise. Destaco, também, que  utilizaremos todas os registros realizados, independente se o auto não foi expedido por motivos técnicos ou administrativos.
...
$$H_{0}\ =\ A \ sequência \ de \ dados \ é \ aleatória \\ H_{0}\ =\ A \ sequência \ de \ dados \ não \ é \ aleatória$$
...
```{r}
DescTools::RunsTest(Alameda$Excesso, alternative = c("two.sided"))
```

- **Av. Raul Lopes**

```{r}
DescTools::RunsTest(Shopping$Excesso, alternative = c("two.sided"))
```

- **Av. Maranhão**

```{r}
DescTools::RunsTest(Maranhao$Excesso, alternative = c("two.sided"))
```

- **Av. B. de Castelo Branco**

```{r}
DescTools::RunsTest(Barao$Excesso, alternative = c("two.sided"))
```
...
\tab Outro teste que podemos realizar é o de Kruskal White, com ele iremos analisar a distribuição dos quatro radares buscando definir se o comportamento é parecido ou não. Dessa forma, avalia-se de $H_{0}$ que determina o comportamento semelhante para todos as distribuições em análise ou a $H_{1}$, que determina que ao menos dois objetos de análise possui diferenaça.    
...
broom::tidy(kruskal.test(excessos ~ origem))
```
...
(book?){Nascimento, author = {NASCIMENTO, F, F.}, title = {MODELOS PROBABILíSTICOS PARA
DADOS EXTREMOS:TEORIA E APLICAÇÕES}, adress = {Teresina, PI}, year = {2012}}

(Dissertação){Pinheiro, author = {PINHEIRO, E, C.}, title = {Contribuições em inferência e modelagem de valores extremos}, address = {São Paulo, SP}, year = {2014}}


(TCC?){Lima, author = {StÊnio Rodrigues Lima}, title = {Modelos Dinâmicos para valores extremos de
dados ambientais do Piauí usando a Distribuiç~ao de Pareto Generalizada (GPD)}, adress = {Teresina, PI},year = {2014}}

(TCC?){Braga, author = {de Oliveira Matos Braga, Douglas}, title = {Aplicação da Teoria de Valores
Extremos para índice pluviométrico da cidade de Juiz de Fora - MG}, adress = {Juiz de Fora, MG}, year ={2015}}