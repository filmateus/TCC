...
\tab  A previsão é uma importante ferramenta para o planejamento de qualquer experimento que se queira implantar. Seja na área pública ou privada, ao termos uma noção melhor dos resultados possíveis, pode-se trabalhar para a maximação do lucro ou melhor aplicação de arrecadações de impostos, por exemplo.

\tab Este trabalho, analisa os dados das autuações de 4 radares em Teresina através das medições que realizam ao captar uma infração de excesso de velocidade. Dessa forma, buscaremos realizar uma previsão da arrecadação possível estes radares oferecer ao município. Existem outros fatores que influenciam na arrecadação por autos de trânsito como o precentual de autos que são pagos ou a data de pagamento das penalidades em relação ao dia de vencimento da penalidade, mas não pertencem ao objetivo de análise deste projeto.

\tab As ferramentas estatísticas que utilizamos para a previsão orcamentária corresponde a primeiramente realizar uma descritiva dos dados, identificando possíveis problemas e falhas no dados que possam prejudicar uma previsão eficiente. Em um segundo momento, iremos utilizar alguns modelos ditribuições probalísticos 
...
knitr::include_graphics('D:/OneDrive/DEV/TCC-1/Imagens/1_MINFRA_9_002.jpg')
...
\tab A distribuição Poisson pertence a família das distribuições quantitivas discretas. Seus resultados indicam a probabilidade de evento ocorrer após um período de tempo, segundo ([@CARSSELA e BERGER], 83), "um dos suposições básicas aobre a qual este modelo é desenvolvida é a de que, para pequnos intervalos de tempo, a probabilidade de uma chegada é proporcional ao tempo de espera.", em outras palavras, o tempo que se aguardar para um evento influência o resultado.

\tab A distribuição Poisson possui um único parametro, $\lambda$, que corresponde a um parametro de intensidade. Sua função de distribuição é:
...
\tab Os modelos apresentados no capítulo anterior são considerados clássicos e tradicionais. Uma nova perspectiva de análise surgiu com a *Bayesiana*, cujo os valores iniciais são originados por distribuições incertas e descritas por uma função de densidade 
...
knitr::include_graphics('D:/OneDrive/DEV/TCC-1/Imagens/Alameda1.png')
```
...
knitr::include_graphics('D:/OneDrive/DEV/TCC-1/Imagens/Maranhao1.png')
```
...
knitr::include_graphics('D:/OneDrive/DEV/TCC-1/Imagens/Shopping1.png')
```
...
knitr::include_graphics('D:/OneDrive/DEV/TCC-1/Imagens/Barao1.png')
```
...
@article{@CARSSELA e BERGER,
  title = {Inferência Estatística},
  author = {CASSELA, G., BERGER, L. R.},
  adress = {SÃO PAULO - SP},
  year ={2020}
  }
