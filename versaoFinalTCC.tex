...
\hspace*{1cm} A previsão é uma importante ferramenta para o planejamento
de qualquer experimento que se queira implantar. Seja na área pública ou
privada, ao termos uma noção melhor dos resultados possíveis, pode-se
trabalhar para a maximação do lucro ou melhor aplicação de arrecadações
de impostos, por exemplo.

\hspace*{1cm} Este trabalho, analisa os dados das autuações de 4 radares
em Teresina através das medições que realizam ao captar uma infração de
excesso de velocidade. Dessa forma, buscaremos realizar uma previsão da
arrecadação possível estes radares oferecer ao município. Existem outros
fatores que influenciam na arrecadação por autos de trânsito como o
precentual de autos que são pagos ou a data de pagamento das penalidades
em relação ao dia de vencimento da penalidade, mas não pertencem ao
objetivo de análise deste projeto.

\hspace*{1cm} As ferramentas estatísticas que utilizamos para a previsão
orcamentária corresponde a primeiramente realizar uma descritiva dos
dados, identificando possíveis problemas e falhas no dados que possam
prejudicar uma previsão eficiente. Em um segundo momento, iremos
utilizar alguns modelos ditribuições probalísticos
...
Além da área citada, podemos encontrar estudos que envolvem valores
extremos em áreas de pesquisa como engenharia de materiais, controle de
tráfego e economia.
...
possuem ficam mais difíceis de prever. Em
(\protect\hyperlink{ref-PINHEIRO}{\textbf{PINHEIRO?}}), podemos
encontrar um resumo da teoria de valores extremos

\textbf{O objetivo da teoria estatística do valor extremo é analisar
valores extremos observados e prever possíveis valores ainda mais
extremos, é fazer inferência para eventos cujas probabilidades são
menroes do que qualquer evento observado anteriormente. Fenômenos em que
a probablidade de um valor extremo é relativamente alta são
caracterizados por distribuições com caudas pesadas}

\newpage

\hypertarget{apresentando-os-dados}{%
\section{Apresentando os dados}\label{apresentando-os-dados}}
...
especificados, no entanto, para a realização do registro infração, é
preciso que a via esteja sinalizada informando a velocidade máxima.

\hspace*{1cm} Um ponto importante sobre as infrações de velocidade é a
existência de uma margem de erro para medição da velocidade registrada.
Para fim de registro de infração, não se utiliza a velocidade obtida
pelo instrumento de medição, mas subtrai-se um valor na velocidade
medida (para medições até 100km/h, o valor reduzido é de 7 km/h). A
velocidade final obtida após a subtração será a considerada para o
registro ou não da infração.

\hspace*{1cm} Por exemplo, para vias com velocidade máxima de 60 km/h,
as infrações serão registradas somente a velocidade calculada no ponto
de medição for, pelo menos, de 68 km/h, registrando um excesso de 1km/h,
mas para uma velocidade registrada de 67 km/h, o valor considerado é de
60km/h, não caracterizando uma infração. O resultado dessa distinção é o
uso de dois termos: \textbf{Velocidade medida}, que consiste na
velocidade Registrada e \textbf{Velocidade Considerada}, referente a
velocidade medida menos o fator de correção. Na \texttt{Figura\ 1}
abaixo, temos a relação entre velocidade medida e velocidade
considerada. A infração é feita quando a considerada é maior que
permitida na via.

\begin{figure}[H]

{\centering \includegraphics[width=4.74in,height=0.93\textheight]{Imagens/1_MINFRA_9_002} 
...
\hspace*{1cm} Estes radares são bastante conhecidos no município de
Teresina com velocidade máxima de 60 km/h, exceção do radar da Av. Barão
de Castelo Branco, onde o excesso é de 40 km/h. Seus bancos de dados são
extensos e iremos apresentar algumas medidas descritivas para termos uma
noção melhor da distribuição dos excessos.
...
pode-se aprofundar neste ponto. Destaco, também, que utilizaremos todas
os registros realizados, independente se o auto não foi expedido por
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.1101}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.1743}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2477}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.1927}}@{}}
\toprule()
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.3731}}
...
\caption{Distribuição de excesso na Alameda Parnaíba}\label{fig:unnamed-chunk-9}
\end{figure}
...
\caption{Distribuição de excesso na Av. Maranhão}\label{fig:unnamed-chunk-10}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=432px]{versaoFinalTCC_files/figure-latex/unnamed-chunk-11-1} 

}

\caption{Distribuição de excesso na Av. B. Castelo Branco}\label{fig:unnamed-chunk-11}
\end{figure}

\newpage

\hypertarget{alguns-testes}{%
\subsection{Alguns testes}\label{alguns-testes}}

\hspace*{1cm} Através das dos gráficos da distribuição dos excessos de
velocidade, podemos afirmar que não ocorrem distribuições do tipo
normal. Nota-se que quanto maior for a velocidade de excesso, menor é
sua probabilidade de ocorrer, sendo a de 1km/h a mais frequente.

\hspace*{1cm} Iremos realizar alguns testes que buscam identificar se a
ocorrência de multas dos excessos ocorre de uma forma aleatória ou não.
Para isso, utilzaremos o teste de corridas, que se baseia no aumento ou
diminuição da medição comparada a anterior.

\hypertarget{teste-de-corrridas}{%
\subsubsection{Teste de Corrridas}\label{teste-de-corrridas}}

\hspace*{1cm} O primeiro teste que iremos realizar tem como objetivo
analisar se a distribuição em análise possui um comportamento aleatório
ou não. De acordo com a resposta obtida por radar, podemos ter uma noção
do funcionamento dos radares na registro do excessos.

\hspace*{1cm} O \texttt{teste\ de\ corridas} faz uma comparação do valor
medido numa posição no vetor em análise, comparando com a medição
anterior analisando se o valor aumentou ou diminuiu e, a partir da
sequencia de aumentos ou diminuições, será obtido o número de corridas,
sendo este o valor utilizado para determinar a aleatoriedade ou não do
processo.\\
\hspace*{1cm} As respostas possíveis do teste aplicado são,

\[H_{0}\ =\ A \ sequência \ de \ dados \ é \ aleatória\]
\[H_{1}\ =\ A \ sequência \ de \ dados \ não \ é \ aleatória\]

\hspace*{1cm} Para a realização do teste, utlizando a função
\texttt{DescTools::RunsTest()} no \texttt{R} e os resultados são
...
\midrule()
\endhead
Alameda Parnaíba & 0.004 \\
Av. Raul Lopes & 0.009 \\
Av. Maranhão & 0.002 \\
Av. Barão de C. Branco & 0.028 \\
\bottomrule()
\end{longtable}

\hspace*{1cm} Com os testes realizados, podemos afirmar que o registro
dos radares não é aleatório para todos os radares. Dessa forma, podemos
contruir uma análise que possa descrever o funcionamento dos radares.

\hypertarget{teste-de-kruskal-white}{%
\subsubsection{Teste de Kruskal White}\label{teste-de-kruskal-white}}

\hspace*{1cm} Outro teste que podemos realizar é o de Kruskal White, com
ele iremos analisar a distribuição dos quatro radares buscando definir
se o comportamento é parecido ou não. Dessa forma, avalia-se de
\(H_{0}\) que determina o comportamento semelhante para todos as
distribuições em análise ou \(H_{1}\), que determina que ao menos dois
objetos de análise possui diferenaça.

\hspace*{1cm} Se o resultado indicar que existe diferença, utilizaremos
um pós-teste (Teste de Dunn) para identificar entre quais ocorrem a
diferença.
...
\hspace*{1cm} O resultado do teste indica que existe diferença entres as
distribuições entre os radares. Assim, o próximo o passo é realizar o
pós teste para identificar onde estão as diferenças.

\hypertarget{puxf3s-teste}{%
\subsubsection{Pós-Teste}\label{puxf3s-teste}}

\hspace*{1cm} Utilizamos os teste de Dunn
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 16\tabcolsep) * \real{0.0800}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 16\tabcolsep) * \real{0.1733}}@{}}
\toprule()
\begin{minipage}[b]{\linewidth}\raggedright
.y.
...
\hspace*{1cm} O pós teste indica que somente Alameda Parnaíba e Av.
Maranhão possuem um comportamento de distribuição parecidos. Em seguida,
fazemos um gráfico Boxplot comparado as distribuições dos excessos.
...
{\centering \includegraphics[width=432px]{versaoFinalTCC_files/figure-latex/unnamed-chunk-15-1} 

}

\caption{Boxplot comparativo das distribuições dos excessos}\label{fig:unnamed-chunk-15}
\end{figure}
\newpage
...
\hspace*{1cm} A distribuição Poisson pertence a família das
distribuições quantitivas discretas. Seus resultados indicam a
probabilidade de evento ocorrer após um período de tempo, segundo
((\protect\hyperlink{ref-CARSSELA}{\textbf{CARSSELA?}} e BERGER), 83),
``um dos suposições básicas aobre a qual este modelo é desenvolvida é a
de que, para pequnos intervalos de tempo, a probabilidade de uma chegada
é proporcional ao tempo de espera.'', em outras palavras, o tempo que se
aguardar para um evento influência o resultado.

\hspace*{1cm} A distribuição Poisson possui um único parametro,
\(\lambda\), que corresponde a um parametro de intensidade. Sua função
de distribuição é:

\[P(X = x|\lambda) = \frac{e^{-\lambda}\lambda^{x}}{x!},\ x = 0,1,...\]

\hspace*{1cm} Para esta distribuição, temos que \(E(X) = \lambda\) e
\(VAR(X) = \lambda\)

\hypertarget{distribuiuxe7uxe3o-pareto-generalizada-gpd}{%
\subsection{Distribuição Pareto Generalizada
...
\subsubsection{Domínio da atração}\label{domuxednio-da-atrauxe7uxe3o}}
...
\subsubsection{Determinação do
Limiar}\label{determinauxe7uxe3o-do-limiar}}
...
\subsubsection{Estimação da GPD}\label{estimauxe7uxe3o-da-gpd}}
...
\hspace*{1cm} Os modelos apresentados no capítulo anterior são
considerados clássicos e tradicionais. Uma nova perspectiva de análise
surgiu com a \emph{Bayesiana}, cujo os valores iniciais são originados
por distribuições incertas e descritas por uma função de densidade
...
{\centering \includegraphics[width=350px]{Imagens/Alameda1} 

}

\caption{Alameda Parnaíba - Função fggpd}\label{fig:unnamed-chunk-17}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=350px]{Imagens/Alameda2} 

}

\caption{Alameda Parnaíba - Função fmgpd}\label{fig:unnamed-chunk-18}
\end{figure}
...
{\centering \includegraphics[width=350px]{Imagens/Maranhao1} 

}

\caption{Av. Maranhão - Função fggpd}\label{fig:unnamed-chunk-20}
\end{figure}
\begin{figure}

{\centering \includegraphics[width=350px]{Imagens/Maranhao2} 

}

\caption{Av. Maranhão - Função fmgpd}\label{fig:unnamed-chunk-21}
\end{figure}
...
{\centering \includegraphics[width=350px]{Imagens/Shopping1} 

}

\caption{Av. Raul Lopes - Função fggpd}\label{fig:unnamed-chunk-23}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=350px]{Imagens/Shopping2} 

}

\caption{Av. Raul Lopes - Função fmgpd}\label{fig:unnamed-chunk-24}
\end{figure}
...
{\centering \includegraphics[width=350px]{Imagens/Barao1} 

}

\caption{Av. Barão - Função fggpd}\label{fig:unnamed-chunk-26}
\end{figure}
\begin{figure}

{\centering \includegraphics[width=350px]{Imagens/Barao2} 

}

\caption{Av. Barão - Função fmgpd}\label{fig:unnamed-chunk-27}
\end{figure}
...
(\protect\hyperlink{ref-article}{\textbf{article?}})\{(\protect\hyperlink{ref-BRAGA}{\textbf{BRAGA?}}),
title = \{Aplicação da Teoria de Valores Extremos para índice
pluviométrico da cidade de Juiz de Fora - MG\}, author = \{BRAGA, D. O.
M.,\}, adress = \{Juiz de Fora, MG\}, year =\{2015\} \}

(\protect\hyperlink{ref-article}{\textbf{article?}})\{(\protect\hyperlink{ref-CARSSELA}{\textbf{CARSSELA?}})
e BERGER, title = \{Inferência Estatística\}, author = \{CASSELA, G.,
BERGER, L. R.\}, adress = \{SÃO PAULO - SP\}, year =\{2020\} \}

(\protect\hyperlink{ref-article}{\textbf{article?}})\{(\protect\hyperlink{ref-LIMA}{\textbf{LIMA?}}),
title = \{Modelos Dinâmicos para valores extremos de dados ambientais do
Piauí usando a Distribuição de Pareto Generalizada (GPD)\} author =
\{Stênio Rodrigues Lima\}, adress = \{Teresina, PI\}, year = \{2014\} \}

(\protect\hyperlink{ref-article}{\textbf{article?}})\{(\protect\hyperlink{ref-NASCIMENTO}{\textbf{NASCIMENTO?}}),
title = \{MODELOS PROBABILíSTICOS PARA DADOS EXTREMOS:TEORIA E
APLICAÇÕES\}, author = \{NASCIMENTO, F, F.\}, adress = \{Teresina, PI\},
year = \{2012\} \}

(\protect\hyperlink{ref-article}{\textbf{article?}})\{(\protect\hyperlink{ref-PINHEIRO}{\textbf{PINHEIRO?}}),
title = \{Contribuições em inferência e modelagem de valores extremos\},
author = \{PINHEIRO, E, C.\}, address = \{São Paulo, SP\}, year =
\{2014\} \}
